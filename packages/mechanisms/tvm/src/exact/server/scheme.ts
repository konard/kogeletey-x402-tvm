 ...
 * Handles price parsing and payment requirements enhancement for Everscale.
 */
export class ExactTvmScheme implements SchemeNetworkServer {
   ...
   * Enhances base requirements with TVM-specific data.
   *
  ...
  enhancePaymentRequirements(
    ...
    // For TVM, we may need to add the facilitator's address for gas sponsoring
    // Similar to SVM's feePayer concept
    return Promise.resolve({
      ...paymentRequirements,
      extra: {
        ...paymentRequirements.extra,
        // Include any facilitator-provided extra data
        ...supportedKind.extra,
      },
    })
  }
  ...
  private defaultMoneyConversion(amount: number, network: Network): AssetAmount {
    // Convert decimal amount to token amount (USDC has 6 decimals on Everscale)
